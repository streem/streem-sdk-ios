...
class StreemInitializer {
    ...
    private let streemEnvironment = "sandbox" // for Production, use `prod`.
    ...
    private init() {
        Streem.initialize(delegate: self, appId: appId, streemEnvironment: streemEnvironment) { [weak self] in
