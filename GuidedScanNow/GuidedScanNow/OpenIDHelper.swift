...
class OpenIDHelper {
    ...
    static func login(withAuthState authState: OIDAuthState,
        ...
        authState.performAction { freshAccessToken, _, error in
            ...
            Task {
                do {
                    try await Streem.sharedInstance.updateHostToRegional(companyCode: companyCode)
                } catch let error {
                    print("Error fetching registration response for companyCode: \(companyCode), error: \(error)")
